...
```bash
sam-timememory$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-timememory$ sam local start-api
sam-timememory$ curl http://localhost:3000/
```
...
```bash
sam-timememory$ sam logs -n HelloWorldFunction --stack-name sam-timememory --tail
```
...
```bash
sam delete --stack-name sam-timememory
```
